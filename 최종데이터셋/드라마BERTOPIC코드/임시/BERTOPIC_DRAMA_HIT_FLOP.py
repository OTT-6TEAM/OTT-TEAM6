...
drama_df = pd.read_parquet("텍스트마이닝결과/drama_with_embeddings_final.parquet")
...
try:
    hierarchical_topics_hit = hit_topic_model.hierarchical_topics(texts_hit)
...
# 3) 계층적 토픽 구조
try:
    hierarchical_topics_flop = flop_topic_model.hierarchical_topics(texts_flop)
...
# 모델 저장 (나중에 불러와서 재사용 가능)
hit_topic_model.save(f"{OUTPUT_DIR}/hit_bertopic_model", serialization="pickle")
print(f"  ✓ {OUTPUT_DIR}/hit_bertopic_model/")

flop_topic_model.save(f"{OUTPUT_DIR}/flop_bertopic_model", serialization="pickle")
