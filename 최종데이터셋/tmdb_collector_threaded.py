"""
TMDB TV Series Data Collector - Threaded Version
멀티스레드 방식으로 TMDB API를 통해 TV 시리즈 데이터 수집
"""
...
class TMDBThreadedCollector:
    def __init__(self, api_key: str, max_workers: int = 40):
        ...
        # 진행 상황 저장
        self.checkpoint_file = "checkpoint_threaded.json"
        self.output_file = "tmdb_tv_series_data_threaded.parquet"
    ...
    def _rate_limited_request(self, url: str, params: dict) -> Optional[dict]:
        ...
        with self.rate_limiter:
            ...
            try:
                ...
                time.sleep(1.0 / self.max_workers)
    
    def fetch_discover_page(self, start_date: str, end_date: str, page: int) -> Optional[dict]:
    ...
    def fetch_external_ids(self, series_id: int) -> Optional[dict]:
        """External IDs (IMDB ID 포함) 가져오기"""
        url = f"{self.base_url}/tv/{series_id}/external_ids"
        params = {"api_key": self.api_key}
        return self._rate_limited_request(url, params)
    
    def fetch_credits(self, series_id: int) -> Optional[dict]:
        """크레딧 정보 (배우, 제작진) 가져오기"""
    ...
    def process_series(self, series_data: dict) -> Optional[dict]:
        """개별 TV 시리즈 처리"""
        series_id = series_data.get("id")
        
        # External IDs 가져오기
        external_ids = self.fetch_external_ids(series_id)
        if not external_ids or not external_ids.get("imdb_id"):
            logger.debug(f"Series {series_id} has no IMDB ID, skipping")
            return None
        
        # Credits 가져오기
        credits = self.fetch_credits(series_id)
        if not credits:
    ...
    def collect_date_range(self, start_date: str, end_date: str) -> None:
        """특정 날짜 범위의 데이터 수집"""
        ...
        while page <= 500:  # TMDB 500페이지 제한
            result = self.fetch_discover_page(start_date, end_date, page)
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                futures = [executor.submit(self.process_series, series) for series in results]
                processed_data = [f.result() for f in concurrent.futures.as_completed(futures)]
            
            # 유효한 데이터만 추가
            valid_data = [d for d in processed_data if d is not None]
            range_data.extend(valid_data)
        ...
        # 스레드 안전하게 전체 데이터에 추가
        with self.data_lock:
            self.collected_data.extend(range_data)
        
        logger.info(f"Completed range {start_date} ~ {end_date}. Range total: {len(range_data)}")
    ...
    def run(self, start_date: str = "2005-01-01", end_date: str = "2025-11-30") -> None:
        """메인 수집 프로세스"""
        logger.info("Starting TMDB TV Series collection (Threaded)")
        ...
        for i, (range_start, range_end) in enumerate(remaining_ranges, 1):
            ...
            self.collect_date_range(range_start, range_end)
...
def main():
    ...
    collector = TMDBThreadedCollector(API_KEY, max_workers=40)
    collector.run(start_date="2005-01-01", end_date="2025-11-30")
