...
- 사전 계산된 임베딩(BAAI/bge-large-en-v1.5) 활용
...
# 데이터 로드
movie_df = pd.read_parquet(r"텍스트마이닝결과\movie_with_embeddings_final.parquet")
hit_score_df = pd.read_parquet("hit_score.parquet")

print(f"영화 데이터: {len(movie_df)}개")
print(f"Hit Score 데이터: {len(hit_score_df)}개")

# Left Join
df_merged = movie_df.merge(hit_score_df, on='imdb_id', how='left')
...
# 모델 생성 및 학습
hit_topic_model = create_bertopic_model(hit_n_neighbors, hit_min_cluster, english_stopwords_movie)

print("\n흥행작 BERTopic 모델 학습 중...")

# ★★★ 핵심 수정: c-TF-IDF용 텍스트는 줄거리(overview)만 사용 ★★★
texts_hit_for_ctfidf = df_hit['overview'].tolist()
...
# 모델 생성 및 학습
flop_topic_model = create_bertopic_model(flop_n_neighbors, flop_min_cluster, english_stopwords_movie)

print("\n비흥행작 BERTopic 모델 학습 중...")

# ★★★ 핵심 수정: c-TF-IDF용 텍스트는 줄거리(overview)만 사용 ★★★
texts_flop_for_ctfidf = df_flop['overview'].tolist()
...
# ==========================================================
# 10-1. Representative_Docs에 영화 제목 매핑 (추가 코드)
# ==========================================================
print("\n" + "="*60)
print("Representative_Docs에 영화 제목 매핑 중...")
print("="*60)
...
def map_representative_docs_to_titles(topic_info_df, texts_list, titles_list):
    """
    Representative_Docs의 줄거리를 영화 제목과 매핑
...
df_hit_result.to_parquet(f"{OUTPUT_DIR}/hit_movie_topics.parquet", index=False)
df_hit_result.to_csv(f"{OUTPUT_DIR}/hit_movie_topics.csv", index=False, encoding='utf-8-sig')
print(f"  ✓ {OUTPUT_DIR}/hit_movie_topics.parquet")
print(f"  ✓ {OUTPUT_DIR}/hit_movie_topics.csv")
...
df_flop_result.to_parquet(f"{OUTPUT_DIR}/flop_movie_topics.parquet", index=False)
df_flop_result.to_csv(f"{OUTPUT_DIR}/flop_movie_topics.csv", index=False, encoding='utf-8-sig')
print(f"  ✓ {OUTPUT_DIR}/flop_movie_topics.parquet")
print(f"  ✓ {OUTPUT_DIR}/flop_movie_topics.csv")
...
# ==========================================================
# 11. BERTopic 모델 저장 (선택적)
# ==========================================================
print("\n" + "="*60)
print("BERTopic 모델 저장 중...")
print("="*60)

# safetensors 사용 + embedding_model 제외 (용량 절약)
try:
...
■ 분석 개요
  ...
  - 임베딩: 장르+줄거리 (BAAI/bge-large-en-v1.5)
...
[데이터 파일 - CSV/Parquet]
  흥행작:
    - hit_movie_topics.csv/parquet : 각 영화의 토픽 할당 결과
    - hit_topic_info.csv           : 토픽 요약 정보 (문서 수, 대표 키워드)
    - hit_topic_keywords.csv       : 토픽별 상위 20개 키워드 및 점수

  비흥행작:
    - flop_movie_topics.csv/parquet : 각 영화의 토픽 할당 결과
...
5. *_topics_documents.html
   ...
   - 각 점은 하나의 영화, 색상은 할당된 토픽
   - 클러스터링이 잘 되었는지 시각적으로 확인

6. *_movie_topics.csv
   - imdb_id, title: 영화 식별 정보
